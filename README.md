...
- **Vocabulary English glosses (v3.3):** words where a single emoji genuinely cannot convey the meaning — abstract adjectives (كبير، صغير، جميل...) and all pronouns (أنا، أنتَ، هو...) — now show a fading English gloss beneath the word, using the same fade-after-3-meetings system as everything else. A picture can show "elephant"; it can't show "big" or "I" — so here English does the one job emoji can't.
- **Numbers 11 to thousands (v3.3):** Lesson 25 (١١-٩٩: teens, tens, compounds) and Lesson 26 (hundreds, thousands, ordinals) — full counting from eleven through thousands, with the real grammar (singular accusative after 11-99, إضافة with مئة/ألف).
- **Fading English scaffold (v1.7):** English appears only where something is genuinely NEW — stage labels, task instructions, drill feedback, and a one-or-two-word gloss for a word you've never looked up. After you've met an item 3 times, its English disappears permanently and Arabic stands alone. Learning content (vocabulary, ayat, rules, drill answers, the tutor's speech) is never translated — the scaffold is for instructions and first encounters only, then it falls away.
- **النصوص المسموعة — Recorded passages (v3.9):** a reciter (the shaykh or a trusted reader) records themselves reading a full passage from any text, pastes the Arabic, and authors comprehension questions on it. Learners hear the real human voice with the text HIDDEN by default, replay freely, reveal the text only if needed, tap any word for the dictionary, then sit a graded comprehension test on what they heard. This is the closest thing in the app to sitting in a dars.
- **الاستماع — Listening (v3.8):** hear a sentence with NO text shown, then choose what you heard. Replay as often as you like; the text and emoji are revealed only after you answer. Trains the ear separately from the eye — the gap that stops readers from following a live dars. Draws sentences from every lesson you've unlocked.
